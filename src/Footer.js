import { BrowserRouter as Router, Switch, Route, Link } from "react-router-dom";

export const menu = [
    {
        title: 'O nama',
        slug: '/O nama',
        description: 'Manastir Somina...',
        picture: 'https://www.in4s.net/wp-content/uploads/2021/01/manastir-somina.jpg'
    },
    {
        title: 'Proizvodi',
        slug: '/proizvodi',
        description: 'Naši proizvodi...',
        picture: 'https://www.dan.co.me/gfx_bb/Manastir-Preobrazenja-Gospodnjeg-na-Hercegovim-Lukama-na-planini-Somini.jpg'
    },
    {
        title: 'Recepti',
        slug: '/recepti',
        description: 'Naši recepti...',
        picture: 'data:image/jpeg;base64,'
    },
    {
        title: 'Pouke',
        slug: '/pouke',
        description: 'Ava Marko kaže...',
        picture: 'https://images.unsplash.com/photo-1599478182733-bfe07a2ce7a0?ixid=MXwxMjA3fDB8MHxzZWFyY2h8MXx8b3J0aG9kb3glMjBwcmllc3R8ZW58MHx8MHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=600&q=60'
    },
    {
        title: 'Kontakt',
        slug: '/kontakt',
        description: 'Broj telefona...',
        picture: 'https://images.unsplash.com/photo-1523966211575-eb4a01e7dd51?ixid=MXwxMjA3fDB8MHxzZWFyY2h8MTB8fGNvbnRhY3R8ZW58MHx8MHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=600&q=60'
    }
]

const Footer = () => {
    return (
        <div className="w-full bg-blue-700">
            <div className="container mx-auto">
                <footer className=" flex flex-col text-xs py-10 apsolute bottom-0  border-t-8 border-b-8 border-blue-200 rounded-t-3xl sm:text-base">
                    {/* <div className="flex flex-col items-center">
                        {menu.map(item => {
                            return (
                                <Link to={item.slug} className="text-gray-400 text-sm font-semibold m-1 hover:text-blue-400 sm:text-lg">{item.title}</Link>
                            )
                        })}
                    </div>   */}
                    <div className="flex flex-col gap-y-1 text-gray-300 items-center justify-evenly">
                        <p>kontakt:</p>
                        <div className="flex flex-col md:flex-row  items-center justify-evenly md:justify-around">
                            <p>069/555-555</p>
                            <p>manastirsomina@gmail.com</p>
                        </div>
                    </div>

                    <div className="flex justify-around text-blue-300 italic">
                        <p>© All rights reserved</p>
                        <p>Powered by ClickNik©</p>
                    </div>
                </footer>   
            </div>
        </div>
    )
}

export default Footer;