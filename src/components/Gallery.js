...
const Gallery = () => {
	...
	const test = (index) => {
		// TODO: rename test to another name
		setIndex(index);
	...
	return (
		<div className="container mx-auto">
			...
			<div className="mt-28 px-4 mb-2 sm:hidden">
				<a href="#">
					<img
						onClick={() => handleClick(galleryArray[0], 0)}
			...
			<div
				...
				className={`${
					modal ? '' : 'hidden'
				} absolute top-0 left-0 w-full h-full bg-gray-200 bg-opacity-80`}
			>
					...
					<div className="container relative sm:max-w-sm xs:w-full">
						<div>
							...
							<button
								...
								style={{
									cursor: `${!leftButtonState ? 'pointer' : 'not-alowed'}`,
								}}
							...
							<button
								onClick={() => handleArrowClick(1)}
								style={{
									cursor: `${!rightButtonState ? 'pointer' : 'not-alowed'}`,
								}}
