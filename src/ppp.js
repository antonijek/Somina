...
const Proba = () => {
  ...
  const [desno, setDesno] = useState('desno');
  const [lijevo, setLijevo] = useState('lijevo');

  useEffect(() => {
    setModal(galleryArray[id].imageSource);
    if (id > 3) {
      setDesno('ne moze dalje');
    } else {
      setDesno('desno');
    }
    if (id < 1) {
      setLijevo('ne moze dalje');
    } else {
      setLijevo('lijevo');
    }
  }, [id]);
  ...
  return (
    <>
      ...
      <div className="flex">
        ...
        <img className="w-96" alt="" src={modal} />
        <button
          onClick={() => {
            if (id < 4) {
              setId(id + 1);
